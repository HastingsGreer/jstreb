...
test.describe("Trebuchet Designer", () => {
  ...
  test("should verify range and maximum force ground truth values", async ({
    page,
  }) => {
    await page.goto("/");
  ...
  test("should toggle optimize button and update range", async ({ page }) => {
    ...
    console.log(
      `Initial range: ${initialRange.toFixed(
        1,
      )} ft, Final range: ${finalRange.toFixed(1)} ft`,
    );
  ...
  test("should toggle gentlify button and reduce peak load", async ({
    page,
  }) => {
    await page.goto("/");
    ...
    console.log(
      `Initial peak load: ${initialPeakLoad.toFixed(
        1,
      )} lbf, Final peak load: ${finalPeakLoad.toFixed(1)} lbf`,
    );
