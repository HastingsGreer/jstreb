...
export function calculateCovariance(X, mean) {
  ...
  return X.map((row, i) =>
    X.map(
      (_, j) =>
        row.reduce(
          (sum, _, k) => sum + (X[i][k] - mean[i]) * (X[j][k] - mean[j]),
          0,
        ) /
        (n - 1),
    ),
...
function testFitAndSampleGaussian(X, numSamples = 10000) {
  ...
  const meanDiff = Math.sqrt(
    mean.reduce((sum, m, i) => sum + (m - sampleMean[i]) ** 2, 0),
  );
  const covDiff = Math.sqrt(
    cov.reduce(
      (sum, row, i) =>
        sum +
        row.reduce((rowSum, c, j) => rowSum + (c - sampleCov[i][j]) ** 2, 0),
      0,
    ),
  );
  ...
  if (meanDiff < meanTol && covDiff < covTol) {
    ...
    console.log(
      "Test failed: Sample statistics deviate too much from expected values",
    );
  }
}
